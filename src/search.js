...
function productListing(value) {
    ...
    if (value) {
        ...
        let relevantProducts = items.filter((item) => {
    ...
    relevantProducts.forEach(function (product) {
        ...
        item.setAttribute('class','flex items-center gap-4 p-2 hover:bg-slate-100')
        item.setAttribute('href','#');
        let productImage = document.createElement('img');
        productImage.setAttribute('src', product.image);
        productImage.setAttribute('alt', product.title);
        productImage.setAttribute('class', 'h-8 w-8 object-cover');

        let content = document.createTextNode(product.title);
...
function enteredValue(ev) {
    let valueEntered = ev.target.value;
    valueEntered = valueEntered.trim().toLowerCase();

    if (enteredValue) {
        productListing(valueEntered);//product listing has been called with users' value//
    }
